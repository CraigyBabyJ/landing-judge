...
(function () {
  ...
  function showAnimatedNumber(score, level, durationMs, quote = '', audioUrl = '', effects = null) {
    ...
    if (audioUrl && ttsEnabled) {
      ...
      // Aggressively try to resume audio context
      ...
      quoteAudio.play()
        .then(() => {
          // Check context state again and try to resume if needed - critical for OBS
          try { if (audioCtx && audioCtx.state !== 'running') { audioCtx.resume().catch(() => {}); } } catch (e) {}
          ...
          try {
            setTimeout(() => {
              ...
              // Final check to ensure context is running
              try { if (audioCtx && audioCtx.state !== 'running') { audioCtx.resume().catch(() => {}); } } catch (e) {}
            }, 150);
          } catch (e) { /* ignore */ }
        })
        ...
        if (audioCtx && audioCtx.state === 'suspended') {
          audioCtx.resume().then(() => {
             // Retry playback if resume succeeds?
             console.log('Resumed audio context after failure');
          }).catch(() => {});
        }
