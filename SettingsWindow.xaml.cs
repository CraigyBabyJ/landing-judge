    ...
    private async Task LoadVoicesForProviderAsync()
    {
            ...
            try
            {
                var voices = await Task.Run(() =>
                {
                    ...
                    return synth.GetInstalledVoices()
                        ...
                        .OrderBy(v => 
                        {
                            var name = v.VoiceInfo.Culture.Name;
                            if (name.StartsWith("en-GB", StringComparison.OrdinalIgnoreCase)) return 0;
                            if (name.StartsWith("en", StringComparison.OrdinalIgnoreCase)) return 1;
                            return 2;
                        })
                        .ThenBy(v => v.VoiceInfo.Culture.Name)
        ...
        if (provider == "Edge")
        {
            try
            {
                ...
                var voices = voicesList
                    ...
                    {
                        var loc = v.Locale ?? "";
                        if (loc.StartsWith("en-GB", StringComparison.OrdinalIgnoreCase)) return 0;
                        if (loc.StartsWith("en", StringComparison.OrdinalIgnoreCase)) return 1;
                        return 2;
                    })
                    .ThenBy(v => v.Locale)
                ...
                {
                    VoiceCombo.Items.Add(new ComboBoxItem { Content = "en-GB-MaisieNeural", Tag = "en-GB-MaisieNeural", IsSelected = true });
                }
            }
            catch (Exception ex)
            {
        ...
        try
        {
            ...
            var voices = response.Voices
                .OrderBy(v => 
                {
                    var code = v.LanguageCode?.ToString() ?? "";
                    if (code.StartsWith("en-GB", StringComparison.OrdinalIgnoreCase)) return 0;
                    if (code.StartsWith("en", StringComparison.OrdinalIgnoreCase)) return 1;
                    return 2;
                })
